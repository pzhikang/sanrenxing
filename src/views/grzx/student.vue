  ...
  <div>
    ...
    <div class="grzltc" :class="{ 'moved': isMoved, 'small-margin-left3': isMarginLeftSmall }">
      ...
      <div class="profile-content">
        ...
        <div class="profile-item">
          <p>学院</p>
          <input type="text" v-model="userInfo.college" placeholder="请输入学院名称" />
        </div>
        <div class="profile-item">
          <p>专业</p>
        ...
        <div class="profile-item">
          <p>入学年份</p>
          <input type="text" v-model="userInfo.enrollment_year" placeholder="请输入入学年份" />
        </div>
        <div class="profile-item">
          <p>学号</p>
          <input type="text" v-model="userInfo.student_id" placeholder="请输入学号" />
        </div>
        <div class="profile-item">
          <p>真实姓名</p>
    ...
    async updateUserInfo() {
      try {
        ...
        const response = await axios.post(
          'https://sanrenxing.wihealth.com.cn/Login/update_user_info/',
          {
        ...
        if (error.response) {
          ...
          if (error.response.status === 403) {
            alert('权限不足或会话已过期，请重新登录');
            this.logout();
          } else {
            alert(`更新失败: ${error.response.data.message || '请稍后再试'}`);
          }
